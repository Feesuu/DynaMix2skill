...
ROLLOUT_DISABLE_RESPONSE_CACHE="${ROLLOUT_DISABLE_RESPONSE_CACHE:-true}"
...
EBST_RUNTIME_FIX_SOURCE_DELTA_MANIFEST="${EBST_RUNTIME_FIX_SOURCE_DELTA_MANIFEST:-}"
...
DYNAMIC_TRAJECTORY_SOURCE="${DYNAMIC_TRAJECTORY_SOURCE:-fixed_replay}"
...
cmd=(
  ...
  "--dynamic-trajectory-source" "$DYNAMIC_TRAJECTORY_SOURCE"
  ...
  "--rollout-disable-response-cache" "$ROLLOUT_DISABLE_RESPONSE_CACHE"
...
if [[ -n "$EBST_RUNTIME_FIX_SOURCE_DELTA_MANIFEST" ]]; then
  cmd+=(
