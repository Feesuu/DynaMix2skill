...
- Recorded no-skill heldout context:
  `82/200 = 41.0%`. The arithmetic difference is `+7.5` percentage points,
  but it is not a matched method delta because endpoint/workers differ and the
  vanilla run completed only 197/200 agent executions.
...
### H1: Global split granularity may weaken experience quality

`min_split_size = 4` may permit many small communities and a deep chain of
lossy summaries. Increasing it to 8 or 10 changes three coupled behaviors: the
ordinary-layer stop, the GMM K search below the threshold, and L0 over-budget
refinement versus token-packing fallback. It is therefore a global granularity
intervention, not a pure tree-depth ablation.
...
distributions, L0 refinement/fallback events, card redundancy, and heldout task
flips.
...
diagnostics. Select without heldout labels, then evaluate at most one selected
setting on the full 200-task development heldout.
...
- Evaluation gate: select a candidate using predeclared build-only metrics,
  then run one full 200-task evaluation with no intermediate heldout tuning.
...
Stop a branch when a fair controlled diagnostic shows no signal twice, when the
